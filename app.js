...
const TRANSLATIONS = {
    en: {
        ...
        imageUrlLabel: '🔗 Product Page Link or Image URL',
        imageUrlPlaceholder: 'https://amazon.com/product/... or https://site.com/image.jpg',
        imageUrlHint: '💡 Paste a product page link — title, description and image will be auto-filled!',
    ...
    tr: {
        ...
        imageUrlLabel: '🔗 Ürün Sayfası Linki veya Görsel URL',
        imageUrlPlaceholder: 'https://trendyol.com/urun/... veya https://site.com/resim.jpg',
        imageUrlHint: '💡 Bir ürün sayfası linki yapıştırın — başlık, açıklama ve resim otomatik doldurulur!',
...
async function fetchProductInfoFromUrl(url) {
    showFetchStatus(state.lang === 'tr' ? 'Sayfa bilgileri alınıyor...' : 'Fetching page info...', 'loading');

    // Strategy 1: microlink.io — handles JS-rendered pages (Trendyol, Hepsiburada, etc.)
    try {
        ...
        if (res.ok) {
            ...
            const badTitles = ['404', 'bulunamadı', 'not found', 'sayfa bulunamadı', 'hata'];
            ...
            if (json.status === 'success' && json.data && !isErrorPage && !isBadTitle) {
                ...
                if (filled) {
                    showFetchStatus(
                        state.lang === 'tr' ? '✅ Ürün bilgileri otomatik dolduruldu!' : '✅ Product info auto-filled!',
                        'success'
                    );
    ...
    // Strategy 2: allorigins.win proxy — parse raw HTML meta tags (works for static sites)
    try {
        ...
        if (filled) {
            ...
            showFetchStatus(
                state.lang === 'tr' ? '⚠️ Bilgi bulunamadı — bu site JS ile yüklendiğinden bilgiler çekilemiyor. Lütfen manuel doldurun.' : '⚠️ Could not extract info from this page. Please fill manually.',
                'warn'
            );
        }
        ...
        showFetchStatus(
            state.lang === 'tr' ? '⚠️ Bilgi çekilemedi — lütfen bilgileri manuel olarak doldurun.' : '⚠️ Could not fetch info — please fill manually.',
            'warn'
        );
        setTimeout(hideFetchStatus, 5000);
    }
